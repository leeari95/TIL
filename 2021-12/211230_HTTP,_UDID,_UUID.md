# 211230 HTTP, UDID, UUID
# TIL (Today I Learned)


12월 30일 (목)

## 학습 내용
- HTTP 활동학습
- 은행창구매니저 STEP4 진행

&nbsp;

## 고민한 점 / 해결 방법

**[개발자 도구에서 헤더 구경하기]**

* 크롬에서 F12누르고 네트워크 탭에 가면 볼 수 있다.
* ![](https://i.imgur.com/Qv6XS1E.png)
* 요청헤더, 응답헤더 등등 어떻게 데이터를 주고받는지 볼 수 있다.

**[포스트맨에서 가짜 API 맛보기]**

* Mock Servers 버튼을 누르면 가짜 API를 구현해볼 수 있다.
* ![](https://i.imgur.com/mj12CRI.png)
* SET이나 POST를 사용해서 example도 만들어볼 수 있다.
    * 그래봤자 Mock일뿐...
* 이것저것 뒤져가며 활동학습 마지막 퀴즈를 풀어보았다.

**[API 만들기 퀴즈]**

* ### 신규 사용자 를 등록한다. 사용자는 프로필 이미지(파일), 아이디, 비밀번호, 닉네임을 갖는다.
    * 이미지는… json에 담으려고 base64로 인코딩했어요.
```
`POST` `http://ios-4th.yagom-academy.kr/users`
`application/json`
----------------------
`{"name": “Ari, "id": "leeari95", "passward": "123456", ”profile_image": “”, }`
```
* ### 등록된 특정 사용자 의 데이터를 조회한다.
```
`GET` `http://ios-4th.yagom-academy.kr/users/1`
```
* ### 등록된 특정 사용자 의 모든 데이터를 수정한다. 
> 바디안에 포함되지 않은 기존 정보들은 날라간다.
```
`PUT` `http://ios-4th.yagom-academy.kr/users/1`
`application/json`
----------------------
`{"name": "Arijjang", "id": "leeari95", "age": 20}`
```
* ### 등록된 특정 사용자 의 일부 데이터를 수정한다.
```
`PATCH` `http://ios-4th.yagom-academy.kr/users/1`
`application/json`
----------------------
`{"name": "Ari", "id": "leeari95", "age": 20}`
```
* ### 등록된 특정 사용자 를 삭제한다.
```
`DELETE` `http://ios-4th.yagom-academy.kr/users/1`
```
* ### 등록된 모든 사용자 를 조회한다. 검색 조건 아이디, 닉네임 등을 이용하여, 만족하는 사용자만 검색할 수 있다.
```
`GET` `http://ios-4th.yagom-academy.kr/users?id=leeari95&name=Ari&age=20`
```

**[퀴즈 풀면서 궁금했던 점]**

* 궁금했던 의문점들을 `노루`가 세심하게 답변해주셔서 기록...!
* 어.. 비밀번호면 https여야하지 않나?
    * 비밀번호는 https 라는 인식은 조금 인터넷 사용팁에 가까운 질문이다.
    * http와 https를 나누는 차이는 uri명세에서는 드러나지 않는다.
    * http보다 https가 좀더 안전하고 신뢰할 수 있지만, 기본적으로 http/https 모두 동일한 프로토콜 규격으로 여기면 되겠다.
* 이미지를... 파일로? Content-Type을 뭘로 해야하지... JSON으로 꾸역꾸역...
    * 이미지를 base64로 인코딩 해서 사용하는 기법은 과거에 인스타에서도 사용했던 기법이다.
    * 이미지를 로딩할 때 네트워크의 영향을 덜 받기 때문에, 범용서비스에서는 부가적인 조치로 사용하는 방법이지만 일반적으로 파일의 경우 `multipart/form-data`를 Content-Type으로 사용한다.
    * base64 String을 파라미터로 주고 받으려면 mimetype을 포함해야 한다. `data:image/png:base64` 이런 단어인데 보통 base64를 다시 디코딩하기 위해서 레스터 포맷을 추론할 수 있어야하는데, 이 포맷이 없는 경우 해당 이미지 데이터가 올바른 색 정보를 갖을 수 없기 때문에 흑백으로 표기되거나 이미지가 깨지는 현상을 막을 수 있다.

---

**[활동학습 들으면서 메모했던 것]**

* 편지봉투는 반드시 있어야한다. (서버에게 요청시 양식)
* 클라이언트(나)는 데이터를 알 수 없다.
* 컨텐츠 타입은 바이너리 파일들을 기존의 시스템에서 문제 없이 전달하기 위해서는 텍스트 파일로 변환이 필요하게 되었는데, 이러한 텍스트 파일로 변환을 인코딩이라고 하며 텍스트 파일을 바이너리 파일로 변환하는 과정을 디코딩이라고 한다.
* 인코딩한 파일은 Content-Type 정보를 파일의 앞부분에 담게 되며, Content-Type은 여러가지 타입이 있다.
    * Content-Type에는 Key에는 메세지 바디의 타입을 나타내는데, 우리가 사용하는 JSON은 application/json 이라고 설정해주면 된다.
* 타입을 어떻게 알려줄까?
    * Content-Type 정보를 파일 앞부분에 담는다.
    * 브라우저의 경우 응답/요청시 HTTP 메세지의 헤더에 정보를 담아서 보내게 되는데 이 헤더에 Content-Type 정보를 담아서 어떤 데이터 종류인지 알려준다.
* MIME-Type 구조
    * type/subtype 으로 구성된다. (소문자 사용가능)
    * type은 1.개별 혹은 2.멀티파트 타입이 될 수 있다.
* 개별파트
    * text : 텍스트를 포함한 모든 문서 (인간 readable)
    * image : 모든 종류의 이미지 (gif 포함)
    * audio : 모든 종류의 오디오 파일
    * video : 모든 종류의 비디오 파일
    * application : 모든 종류의 바이너리 데이터
        * text/plain 서브타입이 없는 텍스트 파일을 위한 기본값 서브타입이 있는 파일은 서브타입 명시되어야 올바르게 감지함.
        * application/octet-stream 서브타입 없는 바이너리 데이터/문서의 기본값 알려지지 않은 파일 타입은 이 타입을 사용해야 합니다. 웹브라우저가 실행여부를 묻기도 하는데 content-Disposition 헤더가 값 attachment와 함께 설정되었고 'Save As'파일을 제안하는지 여부에 따로 브라우저가 그것을 다루게 된다.

---

**[UDID/UUID]**

* UDID(Unique Device Identifier)
    * 기기당 고유한 디바이스 식별 값(불변)
    * 확인방법
        * `Xcode > Window > Shift + CMD + 2`
        * identifier 값 확인
* UUID(Universally uniqye identifier)
    * 8-4-4-4-12라는 5개의 그룹을 '-'(하이픈)으로 구분
        * 실행할 때마다 변하는 값
    * 10^40개 이므로 앱을 실행할 때 마다 랜덤 값을 부여
        * ex) 12345678–1234–abcd–1234–123456789012
    * 출력
        * `print(UUID())`
    * 보통 고유한 값이 필요할 때 활용한다. 유저를 등록한다던지...등등

---

- 참고링크
    - https://www.youtube.com/watch?v=BEK354TRgZ8
    - https://babbab2.tistory.com/70
